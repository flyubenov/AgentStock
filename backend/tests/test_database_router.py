...
def _patch_deletes(db=True, screener=True, risk_reward=True, tickers=True):
    return (
...
def test_delete_succeeds_when_only_the_database_row_exists():
    ...
    a, b, c, d = _patch_deletes(db=True, screener=False, risk_reward=False, tickers=False)
    with a, b, c, d:
        resp = client.delete("/api/database/AAPL")
    body = resp.json()
    assert body["deleted"] is True
    assert body["removed"] == {"database": True, "screener": False, "risk_reward": False, "tickers": False}


def test_delete_unknown_ticker_reports_not_found():
    a, b, c, d = _patch_deletes(db=False, screener=False, risk_reward=False, tickers=False)
    with a, b, c, d:
