...
2. **Measure the blast radius first — it feeds the next step.** This is mandatory and it is cross-classification. Before proposing *any* change, evaluate its knock-on effect on *other* tickers — not only ones in the same tier, but names in **other** classifications the shared code path also touches (a cap in `models.py`, a guard in `engine.py`, a scoring helper in `scoring.py` runs for everything that reaches it). Run a read-only sweep across the test universe: which tickers move, in which direction, by roughly how much, and confirm the documented canaries and neighbors don't regress. A change whose blast radius you can't characterize is not ready to propose. Agent Stock's memory is full of fixes that over-corrected a neighbor in a *different* category.
3. **Match the engagement to the fix's complexity — decide from the measured blast radius, not from where the code lives.** Living in shared / cross-classification code (`models.py`, `engine.py`, `scoring.py`) does **not** make a change complex — what matters is whether there is a real design decision and how much actually moves.
   - **Simple / localized → just ask in normal conversation** which approach the user prefers, then TDD it. Signals: the fix **mirrors or extends an existing pattern** (e.g. adding a branch that copies a valve the neighbouring band already has), **reuses existing constants** (no new knob), **one obvious design**, and the **measured** blast radius is narrow (one or a few tickers, canaries unmoved). *Example: the `$150B–$1T` fade-band relief — a one-branch mirror of the mega valve, no new constant, PLTR-only — was a normal-session change even though it lives in shared `models.py`. It did not need a brainstorm.*
   - **Complex / structural → open a brainstorming session** (`superpowers:brainstorming`). Signals: a **new mechanism, constant, tier, or guard**; **several genuinely viable designs** to weigh (step vs graduated, which basis to source from); a **broad or hard-to-bound measured blast radius**; or the change **re-opens a documented calibration** across many names. Surface the mechanism, options, and blast radius; let the design emerge with the user.
   - **Escalate mid-conversation if a design fork appears.** A change that looked simple becomes a brainstorm the moment a real fork surfaces — the user questions the underlying mechanism, or two defensible designs diverge. When genuinely unsure *and* you cannot bound the blast radius, treat it as complex.
...
## Reuse an existing pattern before inventing one (do this before designing any fix)

Agent Stock has been tuned over many sessions, and the mechanisms it already contains are the vocabulary you should build the next fix from. **Whenever a solution or fix is needed for an issue you've found, first analyze the codebase for a place that already handles a structurally-similar problem, and see whether the new gap can be solved by reusing or extending that same shape.** A fix that mirrors an existing, already-validated pattern is smaller, more consistent, less likely to regress a neighbor, and easier for the next session to recognize.

**How to look:** name the *shape* of the problem, not the ticker — "I need a cap that grows with the company then flattens", "I need a guard that ignores a leg when a denominator is non-operating", "I need an offset that widens with a financial signal" — then `grep` the engine/models/scoring for that shape (`_ramp`, `_ceiling`, `_cap`, `_frac`, `_hold_years`, `_eligible`, `_guard`). If you find one, extend it or copy its structure; if you must add a new mechanism, say explicitly *why no existing pattern fit*.

**Worked case study — the three-scenario growth band (scenario-growth-band, merged to master 2026-07-21).** The gap: for capped hyper-growers (PLTR), `build_scenarios` collapsed `optimistic == realistic`, so the fair value (the average of the three legs) had false precision and no real bull case. Rather than invent a scenario model from scratch, we found that `_ev_ebitda_ceiling` (`models.py`) already coupled a *multiple* to revenue growth with exactly the wanted shape — **flat floor → linear ramp that grows with revenue → saturating cap** — i.e. a pattern for making a calculation's ceiling dynamic in the company's financial performance. We lifted that shape into a shared `_ramp(g, lo, hi, at_lo, at_hi)` helper and reused it three ways: the optimistic/pessimistic **offsets** ramp with a growth signal, and the optimistic **ceiling** (`_opt_ceil`) saturates by type/size (EARLY 0.50, MEGA 0.28 hard cap, LARGE 0.32→0.35 on FCF/EBITDA quality via `_quality_frac` — itself a copy of `_ev_ebitda_ceiling`'s conversion ramp). Guards were reused too: `_band_growth_signal` keys off the **same** rate the realistic leg uses, so the existing `_earnings_distorted` / `_earnings_non_operating` guards govern the bull leg automatically (BWXT's non-operating growth can't sneak back through optimistic).

### Lessons learned from that session (apply them going forward)

- **Reuse paid off exactly as intended.** Because the band reused the EV/EBITDA-ceiling shape, the whole-branch opus review came back with **zero Critical / zero Important** findings — the mechanism was already understood and validated in its original home.
- **A dynamic ceiling can flip a levered name — temper by the same signal the neighbor already uses.** The flat EARLY_GROWTH 0.50 bull ceiling flipped **CRWV** live from SELL to BUY ($62→$88), undoing the [[crwv-funding-gap-bridge]] fix, because a net-debt-funded burner got the same cash-funded bull case as net-cash **NBIS**. The fix mirrored the funding-gap premise onto the growth-*rate* ceiling: `_leverage_frac` (net_debt/market_cap) ramps the EARLY ceiling 0.50→0.35 as leverage rises. **Always sweep a levered / funding-gap burner (CRWV) into the blast-radius basket when you touch an EARLY_GROWTH or bull-case path** — it was *not* in the 12-name basket and the flip was only caught as a late blast-radius check.
- **Re-derive constants after you correct an input, and check the extremes.** A dynamic ceiling that saturates a realistic cap can legitimately meet realistic at the extreme (a maximally-levered >140%-grower) — that's a *binding economic ceiling*, not the false-precision collapse; document such corners in the docstring instead of forcing an artificial gap (a universal `base+floor` would breach the MEGA/LARGE hard caps).
- **Mirror a helper's guards when you copy it.** `_quality_frac` copied `_ev_ebitda_ceiling`'s conversion ramp but initially dropped its `QUALITY_CONV_HI > QUALITY_CONV_LO` divisor guard; parity was restored in review. When you lift a pattern, lift its guards too.
...
- Routing a **simple mirror-fix to a full brainstorm** because it lives in shared code. Gate on the *measured* blast radius and whether there's a real design choice — not on where the code sits. A one-branch copy of an existing valve, reusing existing constants, that moves one ticker is a normal-session change.
